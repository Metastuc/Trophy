...
export const createStream = async (req: Request, res: Response<TypedResponse<RoomResponse>>): Promise<void> => {
  try {
    ...
    const userSnap = await db.collection("users").where("address", "==", address).limit(1).get();
