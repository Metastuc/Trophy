    ...
    res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to start stream recording",
    });
  }
};
  ...
  try {
    ...
    res.status(500).json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Failed to disable stream recording",
    });
  }
};
...
export const endLivestream = async (
  ...
  try {
    ...
    res.status(500).json({
      status: "error",
      message:
        error instanceof Error ? error.message : "Failed to end livestream",
    });
  }
};
...
export const getRecordingUrl = async (
  ...
  try {
    ...
    res.status(500).json({
      status: "error",
      message:
        error instanceof Error ? error.message : "Failed to get recording URL",
    });
  }
};
