...
export const recordStreamJoin = async (walletAddress: string, roomId: string) => {
    try {
        ...
        const usersSnapshot = await db.collection("users").where("walletAddress", "==", walletAddress).get();
        ...
        await db.collection("livestreams").doc(roomId).collection("joinedUsers").doc(userDocId).set({
