...
export const getProfile = async (req: Request, res: Response<TypedResponse<ProfileResponse>>): Promise<void> => {
    try {
        ...
        const balancePromises = tokens.map(
            async (token: { testnetAddress: string; mainnetAddress: string; symbol: string; decimals: number }) => {
        ...
        tokenBalances = [
            {
                ...
                priceChange24h: ethPriceResponse.raw["24hrPercentChange"]
                    ? ethPriceResponse.raw["24hrPercentChange"]
                    : undefined,
            },
